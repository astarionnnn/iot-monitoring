...
export default function AutomationCard({ sensorData, onRuleTriggered }) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(docRef, (docSnap) => {
            if (docSnap.exists()) {
                ...
                // Initialize with default rules if not exists
    ...
    const updateRule = async (device, updates) => {
        try {
            const docRef = doc(db, "automation", "rules");
            await updateDoc(docRef, {
                [`${device}`]: { ...rules[device], ...updates }
            });
        } catch (error) {
    ...
    const toggleDevice = async (device, status, triggerReason) => {
        try {
            ...
            if (shouldTurnOn) {
                ...
                if (device === "pump" && rule.duration > 0) {
                    ...
                    pumpTimeout.current = setTimeout(async () => {
                        // Re-fetch rule mode before turning off
    ...
    }, [sensorData, rules, loading]);

    const getUnit = (condition) => {
